...
*   **`cogni_scaffold/server.py`**: 一个FastMCP应用，将`CogniScaffold`的功能通过流式HTTP API暴露出来。
*   **`cogni_scaffold/requirements.txt`**: 定义了运行服务器所需的Python依赖（如FastMCP）。
...
*   **测试服务器功能:**
    *   **操作:**
        ...
        2.  **启动服务器:** 从项目根目录(`playgroud`)运行: `cogni_scaffold/.venv/bin/python cogni_scaffold/server.py`。
    *   **目的:** 启动一个本地Web服务器(通常在`http://127.0.0.1:8080`)，你可以使用`curl`或其他HTTP客户端来测试流式API端点。
